...
### 7-14. 「片側パディング」と「継ぎ目の実効余白」を取り違えていた（2026-08-16 の実測）

ブランドオーナー指摘「セクションごとのパディングが大きすぎる。次の見出しが出るまでの間が長い」を
Chromium 実レンダリングで検証した結果、指摘どおりだった。

| 継ぎ目                         | 実測（修正前） | 実測の国内中央値 | 母集団最大（LayerX） |
| ------------------------------ | -------------: | ---------------: | -------------------: |
| CorporateTop（trust）本文同士  |    288〜352px |        96〜104px |               174px |
| 製品LP（product）本文同士      |         256px |        96〜104px |               174px |

原因は2つ。

1. **物差しの取り違え**。`--spacing-section-*` は**セクションの片側**パディングなので、
   隣り合えば `下 + 上` で必ず2倍になる。ところが research-eyebrow.md §4-2 は
   この片側の値（6rem = 96px）を、実測の**継ぎ目の実効余白**（中央値 96〜104px）と
   突き合わせて「一致・変更不要」と結論していた。半分と全体を比べていた
2. **検証対象がページ上に存在しなかった**。§4-2 が検査した既定値 `md` は、
   実際には 22 セクション中 20 が `spacing="lg"` を直書きしていてほぼ使われていない。
   既定値だけを見ると、ページの実物とは無関係な検証になる

#142（muted を実測中央値へ + CorporateTop を `<Page tone="trust">` 配下に）は
色ではなく**トーンの付与**を通じてこれを悪化させた。trust はセクション余白を1段広げるため、
CorporateTop の継ぎ目は 256px → 320px（+25%）になっていた。

**教訓: 余白は「トークンの値」ではなく「隣接したときの実距離」で検証する。**
CSS の宣言値を読むだけでは倍になっていることに気づけない。VRT も基準を撮り直した
時点の見た目を正とするだけで、余白の絶対量が妥当かは判定しない
（§7-13 の「VRT が守る範囲」と同じ限界）。

計測スクリプトは使い捨てだが、やり方は
「Storybook 静的ビルドを `vrt/serve.mjs` で配信 → Playwright で
セクション要素の実座標を取り、`次セクションの見出し top − 前セクションの内容 bottom` を出す」。

### 7-15. ファーストビューの占有は余白では担保できない（2026-08-16 の実測）

§7-14 の余白半減の直後、ブランドオーナーから「ヒーローの下のセクションが
ファーストビューに入ってきて、ヒーローが弱くなった」という指摘が出た。実測すると
CorporateTop（trust・1440×900）でヒーローの占有は 93% → 71%、次セクションの見出しは
993px → 721px と画面内に入っていた。指摘どおりだった。

**ただしパディングを戻すのは筋が悪い。** ヒーローがファーストビューを占有できるかは
コピー量と画面の高さで決まるため、固定値のパディングでは短いコピーのページで破綻する。
実サイト13件を測ると、1画面を占有しているサイトはいずれも高さを視野基準で持っており
（Chatwork `min-height: calc(100% - 48px)`、LayerX はヒーロー高 = 900px）、
**パディングで埋めているサイトは1件も無かった**（docs/research/research-first-view.md）。

対応: `--hero-min-height: calc(92svh - 4rem)` を導入し `HeroSection` の `min-height` に充てた。
残り 8% に次の面を覗かせてスクロールの手がかりを残す。campaign トーンは適用外。

**教訓: 「ファーストビューの強さ」と「セクション間のリズム」は別の変数で持つ。**
片方を触ってもう片方が壊れる構造にしない。今回は余白トークンがその両方を兼ねていた。

計測上の注意として、ヒーロー要素そのものの自動判定（`h1` を含む全幅の祖先を辿る）は
実サイトの4件で 100px 前後の要素を拾って使えなかった。ページ間で比較する指標は
**最初の `h2` の y 座標**のほうが頑健。

### 7-16. VRT 基準は「CI と同じ描画環境」で撮る（2026-08-16）

余白変更に伴う基準の撮り直しで、CI の VRT が18ページ落ちた。差分は 1000px 前後で、
中身は**カードのラベルが豆腐（□）になった基準 PNG** だった。原因は撮影した
サンドボックスに和文システムフォントが1つも無かったこと（`fc-list :lang=ja` = 0）。
Google Fonts の和文はサブセット分割されているため、取りこぼしたサブセットの文字が
フォールバック先を持てず豆腐になる。CI（ubuntu runner）は和文フォントを持つので正しく出る。

`fonts-noto-cjk` を入れて撮り直すと豆腐は消えたが、**CI とは 300〜3800px ずれたまま**だった
（フォールバック先の字形が CI と違う）。つまり撮影環境が違えば基準は一致しない。

対応: CI が出力した実画像（`vrt-diff` アーティファクトの `*-actual.png`）を基準として採用した。
比較スクリプトで CI 画像との差分 0px を確認してからコミットしている。

**教訓: 基準 PNG は CI と同じ環境の描画で作る。** 手元で撮る場合は、
和文フォントの有無と字形が CI と一致していることを先に確かめる。
`assertWebFontsLoaded`（ファミリ単位の検査）はサブセット取りこぼしを検出できない。
